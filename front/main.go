...
func main() {
	...
	r.Path("/user-register").Methods(http.MethodPost).HandlerFunc(fs.UserRegistHandler)
	...
	r.Path("/tag-register").Methods(http.MethodPost).HandlerFunc(auth(fs.TagRegisterHandler))
